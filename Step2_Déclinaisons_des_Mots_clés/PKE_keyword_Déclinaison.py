...
df = pd.read_csv(os.path.join(ROOT_DIR, 'Data', 'PKE_Keyword.csv')) #récupération des données précedament générer dans l'étape 1
...
for index ,row in df.iterrows():
    doc = nlp(row["texte"]) #Creation d'un NLP pour distinguer les entités nommées , les Stops words , Mots clés lematisés
...
df.to_csv(os.path.join(ROOT_DIR, 'Data', 'PKE_Keyword_Déclinaison.csv')) #Enregistrement sous forme de CSV dans le fichier /Data/PKE_Keyword_Déclinaison.csv
print(time.strftime("%H:%M:%S", time.localtime()), end=' : ')
print('processus end')